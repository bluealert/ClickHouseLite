...
public:
    Connection(String host_, UInt16 port_, String default_database_,
               String user_, String password_,
               String client_name_ = "client",
               Protocol::Compression compression_ = Protocol::Compression::Enable,
               Protocol::Encryption encryption_ = Protocol::Encryption::Disable,
               const Poco::Timespan &connect_timeout_ = Poco::Timespan(DBMS_DEFAULT_CONNECT_TIMEOUT_SEC, 0),
               const Poco::Timespan &receive_timeout_ = Poco::Timespan(DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC, 0),
               const Poco::Timespan &send_timeout_ = Poco::Timespan(DBMS_DEFAULT_SEND_TIMEOUT_SEC, 0),
               const Poco::Timespan &sync_request_timeout_ = Poco::Timespan(DBMS_DEFAULT_SYNC_REQUEST_TIMEOUT_SEC, 0))
        :
        host(std::move(host_)), port(port_), default_database(std::move(default_database_)),
        user(std::move(user_)), password(std::move(password_)), resolved_address(host, port),
    ...
    Connection(String host_, UInt16 port_, const Poco::Net::SocketAddress & resolved_address_,
